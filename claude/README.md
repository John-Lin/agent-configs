...
source (`agents-md/AGENTS.base.md`, `AGENTS.personal.md`) and the skills apm
installs into `~/.agents/skills/`. See the top-level README and `docs/ai.md`.

`sync-claude` symlinks `agents/` into `~/.claude/`. `~/.claude/CLAUDE.md` is a
symlink to the canonical `~/.pi/agent/AGENTS.md`; `settings.json` is a generated
file. Do not edit either directly — edit the sources and re-run
`make sync-claude`.

`~/.claude/skills/` is outside make's scope entirely: apm deploys real skill
directories there, and `make clean` leaves them alone.
...
Skills are not stored in this repository and are not installed by make.
`apm install --global` fetches them from their upstream repositories into
`~/.agents/skills/<name>` and deploys a second copy to `~/.claude/skills/<name>`,
the only skill directory Claude Code discovers. See `docs/ai.md`.
